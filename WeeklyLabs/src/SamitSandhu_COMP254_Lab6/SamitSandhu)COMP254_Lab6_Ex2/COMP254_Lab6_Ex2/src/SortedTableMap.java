...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  public boolean containKey(K key)
  {
    ...
    // find an index where the key would go or is at
    int indx = findIndex(key);

    //See if the index is within range to prevent out of bound errors.
    //get the key value pair thats located at that index
    //get the key of that pair and compare if thats the same key -> true else -> false
    if (indx < size() && table.get(indx).getKey().equals(key))
	  return true;
    return false;
  }
